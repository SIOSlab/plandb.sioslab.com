...
def getIPACdata(forceNew=False):
    ...
    composite_cols = [
        col
        for col in pscp_data.columns
        if (col == "pl_name") or str.startswith(col, "st_") or col == "sy_vmag"
    ]
    ...
    merged_data["publication_year"] = merged_data.pl_refname.str.extract(r"(\d{4})")
    merged_data["shortrefs"] = [
        authregex.sub("", merged_data["pl_refname"][j]).strip()
    ...
    for j, name in enumerate(merged_data["pl_name"].values):
        ...
        sorted_rows = planet_rows.sort_values(
            by=["publication_year"], axis=0, ascending=False
        )
        good_idx = sorted_rows.index[0]
        good_lvl = 0
        for index, row in sorted_rows.iterrows():
            base_need = (
                not pd.isnull(row["pl_orbsmax"]) or not pd.isnull(row["pl_orbper"])
            ) and (not pd.isnull(row["pl_bmassj"]) or not pd.isnull(row["pl_radj"]))

            # Has everything
            if good_lvl < 8 and (
                base_need
                and not pd.isnull(row["pl_orbeccen"])
                and not pd.isnull(row["pl_orbtper"])
                and not pd.isnull(row["pl_orblper"])
                and not pd.isnull(row["pl_orbincl"])
            ):
            ...
            # Has either periapsis time or argument of pariapsis
                ...
                and not pd.isnull(row["pl_orbeccen"])
                and (
                    not pd.isnull(row["pl_orbtper"]) or not pd.isnull(row["pl_orblper"])
                )
            ):
                if not pd.isnull(row["pl_radj"]):
                    good_idx = index
            ...
            # Has eccentricity
            elif good_lvl < 2 and (base_need and not pd.isnull(row["pl_orbeccen"])):
                if not pd.isnull(row["pl_radj"]):
                    good_idx = index
                    good_lvl = 2
                elif good_lvl < 1:
    ...
    # return ps_data, pscp_data, merged_data
    keep = (
        (~np.isnan(merged_data["sy_dist"].values))
        & (
            ~np.isnan(merged_data["pl_orbsmax"].values)
            | (
                ~np.isnan(merged_data["pl_orbper"].values)
                & ~np.isnan(merged_data["st_mass"].values)
            )
        )
        & (
            ~np.isnan(merged_data["pl_bmassj"].values)
            | ~np.isnan(merged_data["pl_radj"].values)
        )
    )
    ...
    merged_data.loc[has_lum, "st_lum_correction"] = (
        10 ** merged_data.loc[has_lum, "st_lum"]
    ) ** 0.5  # Since lum is log base 10 of solar luminosity

    # fill in missing smas from period & star mass
    nosma = np.isnan(merged_data["pl_orbsmax"].values)
    p2sma = lambda mu, T: ((mu * T**2 / (4 * np.pi**2)) ** (1 / 3.0)).to("AU")  # noqa
    GMs = const.G * (
        merged_data["st_mass"][nosma].values * u.solMass
    )  # units of solar mass
    T = merged_data["pl_orbper"][nosma].values * u.day
    tmpsma = p2sma(GMs, T)
    merged_data.loc[nosma, "pl_orbsmax"] = tmpsma
    merged_data["pl_calc_sma"] = pd.Series(
        np.zeros(len(merged_data["pl_name"])), index=merged_data.index
    )
    ...
    GMerrs = (
        (
            (merged_data["st_masserr1"][nosma] - merged_data["st_masserr2"][nosma])
            / 2.0
        ).values
        * u.solMass
        * const.G
    ...
    Terrs = (
        (merged_data["pl_orbpererr1"][nosma] - merged_data["pl_orbpererr2"][nosma])
        / 2.0
    ).values * u.day

    smaerrs = np.sqrt(
        (2.0 * T**2.0 * GMs) ** (2.0 / 3.0)
        / (9 * np.pi ** (4.0 / 3.0) * T**2.0)
        * Terrs**2.0
        + (2.0 * T**2.0 * GMs) ** (2.0 / 3.0)
        / (36 * np.pi ** (4.0 / 3.0) * GMs**2.0)
        * GMerrs**2.0
    ).to("AU")
    ...
    WA = np.arctan(
        (merged_data["pl_orbsmax"].values * u.AU)
        / (merged_data["sy_dist"].values * u.pc)
    ).to("mas")
    merged_data["pl_angsep"] = WA.value
    sigma_a = (
        (merged_data["pl_orbsmaxerr1"] - merged_data["pl_orbsmaxerr2"]) / 2.0
    ).values * u.AU
    sigma_d = (
        (merged_data["sy_disterr1"] - merged_data["sy_disterr2"]) / 2.0
    ).values * u.pc
    sigma_wa = (
        np.sqrt(
            (
                (merged_data["pl_orbsmax"].values * u.AU) ** 2.0 * sigma_d**2
                + (merged_data["sy_dist"].values * u.pc) ** 2.0 * sigma_a**2
            )
            / (
                (merged_data["pl_orbsmax"].values * u.AU) ** 2.0
                + (merged_data["sy_dist"].values * u.pc) ** 2.0
            )
            ** 2.0
        ).decompose()
        * u.rad
    ...
    merr = (
        (
            (
                (
                    merged_data["pl_bmassjerr1"][noR].values
                    - merged_data["pl_bmassjerr2"][noR].values
                )
                / 2.0
            )
            * u.M_jupiter
        ...
        if np.isnan(merr[j]):
            ...
            merr_earth = (
                (
                    (
                        (
                            merged_data.iloc[j]["pl_bmasseerr1"]
                            - merged_data.iloc[j]["pl_bmasseerr2"]
                        )
                        / 2.0
                    )
                    * u.M_earth
            ...
            Rerr[j] = (
                forecaster_mod.calc_radius_from_mass(
                    u.M_earth
                    * np.random.normal(loc=m_val, scale=merr_earth, size=int(1e4))
                )
                .std()
                .value
            )
            ...
            Rerr[j] = (
                forecaster_mod.calc_radius_from_mass(
                    u.M_earth
                    * np.random.normal(loc=m_val, scale=merr[j], size=int(1e4))
                )
                .std()
                .value
            )
    ...
    Rf[mg10] = griddata(
        fortney.giant_pts2,
        fortney.giant_vals2,
        (np.array([10.0] * np.where(mg10)[0].size), tmpsmas, tmpmass),
    )
    ...
    tmpsmas = merged_data["pl_orbsmax"][noR].values
    tmpsmaserr = (
        merged_data["pl_orbsmaxerr1"][noR].values
        - merged_data["pl_orbsmaxerr2"][noR].values
    ) / 2.0
    ...
    for j, _ in enumerate(tmpsmas):  # Create smax distribution
        if np.isnan(tmpsmaserr[j]) or (tmpsmaserr[j] == 0):
            ...
            adist[j, :] = np.random.normal(
                loc=tmpsmas[j], scale=tmpsmaserr[j], size=int(1e4)
            )
    ...
    for j, _ in enumerate(m):  # Create m distribution and calculate errors
        ...
        tmpsmas[tmpsmas < fortney.giant_pts2[:, 1].min()] = fortney.giant_pts2[
            :, 1
        ].min()
        tmpsmas[tmpsmas > fortney.giant_pts2[:, 1].max()] = fortney.giant_pts2[
            :, 1
        ].max()

        tmpmass = mdist[mg10_dist]
        tmpmass[tmpmass > fortney.giant_pts2[:, 2].max()] = fortney.giant_pts2[
            :, 2
        ].max()

        Rf_dist[mg10_dist] = griddata(
            fortney.giant_pts2,
            fortney.giant_vals2,
            (np.array([10.0] * np.where(mg10_dist)[0].size), tmpsmas, tmpmass),
        )
    ...
    merged_data.loc[noR, "pl_radj_fortneyerr1"] = (
        (Rf_err * u.R_earth).to(u.R_jupiter)
    ).value
    # merged_data = merged_data.assign(pl_radj_forecastermoderr2=merged_data['pl_radjerr2'].values)
    merged_data["pl_radj_fortneyerr2"] = merged_data["pl_radjerr2"].values
    merged_data.loc[noR, "pl_radj_fortneyerr2"] = -(
        (Rf_err * u.R_earth).to(u.R_jupiter)
    ).value
    ...
    maxWA = WA[:]
    maxWA[hase] = np.arctan(
        (
            merged_data["pl_orbsmax"][hase].values
            * (1 + merged_data["pl_orbeccen"][hase].values)
            * u.AU
        )
        / (merged_data["sy_dist"][hase].values * u.pc)
    ).to("mas")
    ...
    # TODO: investigate I > 90 cases
    Is = merged_data["pl_orbincl"][hasI].values * u.deg
    Is[Is > 90 * u.deg] = 180 * u.deg - Is[Is > 90 * u.deg]
    s[hasI] *= np.cos(Is)
    s[~hasI] = 0
    ...
    ms = MeanStars()
    nolum_teff = np.isnan(merged_data["st_lum"].values) & ~np.isnan(
        merged_data["st_teff"].values
    )
    ...
    nolum_noteff_spect = (
        np.isnan(merged_data["st_lum"].values)
        & ~merged_data["st_spectype"].isnull().values
    )
    ...
    for str_row in spects.values:
        ...
        if spec_num_match is not None:
            spec_num = str_row[
                spec_num_match.start() + 1 : spec_num_match.end() + 1
            ]  # noqa
...
def packagePhotometryData(dbfile=None):
    ...
    tmp = pd.read_sql_table("g25_t150_m0.0_d0.5_NC_phang000", connectionl)
        ...
        for j, d in enumerate(dists):
            ...
            for k, cloud in enumerate(clouds):
                ...
                for l, beta in enumerate(betas):
                    ...
                    try:
                        tmp = pd.read_sql_table(name, connectionl)
                    except:
...
def calcQuadratureVals(orbitfits, bandzip, photdict):
    ...
    for j, (Rp, fe, a, I, e, w, lm) in enumerate(
        zip(Rps, fes, smas, inc, eccen, arg_per, lum)
    ):
        t_bar.update()
        ...
        for c in photdict["clouds"]:
            for l, band, bw, ws, wstep in bandzip:
                if j == 0:
                    ...
                    tmpout["quad_radius_" + "%03dC_" % (c * 100) + str(l) + "NM"] = (
                        np.zeros(smas.shape)
                    )
                ...
                if not np.isnan(e) and not np.isnan(w) and I != 0:
                    ...
                    pphi1 = (
                        quadinterps[float(feinterp(fe))][
                            float(distinterp(r1 / lum_fix))
                        ][c](ws).sum()
                        * wstep
                        / bw
                    )
                    ...
                    tmpout["quad_radius_" + "%03dC_" % (c * 100) + str(l) + "NM"][j] = (
                        a / lum_fix
                    )
...
def genOrbitData(data, bandzip, photdict, t0=None):
    if t0 is None:
        t0 = Time("2026-10-01T00:00:00", format="isot", scale="utc")
    ...
    for j in range(len(plannames)):
        ...
        if np.isnan(Tp):
            ...
            if not np.isnan(Mstar):
                if not np.isnan(row["pl_bmassj"]):
                    mu = const.G * (
                        (Mstar * u.solMass).decompose()
                        + (row["pl_bmassj"] * u.jupiterMass).decompose()
                    )
                else:
                    mu = const.G * (Mstar * u.solMass).decompose()
                Tp = (
                    (2 * np.pi * np.sqrt(((a * u.AU) ** 3.0) / mu))
                    .decompose()
                    .to(u.d)
                    .value
                )
        ...
        if (
            np.isnan(I)
            and np.isnan(row["pl_orbinclerr1"])
            and np.isnan(row["pl_orbinclerr2"])
        ):
            I = np.pi / 2.0
            if row["pl_bmassprov"] == "Msini":
                Icrit = np.arcsin(
                    ((row["pl_bmassj"] * u.M_jupiter).to(u.M_earth)).value
            ...
            IList = np.array(
                [
                    I,
                    I + row["pl_orbinclerr1"] * np.pi / 180,
                    I + row["pl_orbinclerr2"] * np.pi / 180,
                ]
            )
            defs = [1, 0, 0]
            icrit_flag = [0, 0, 0]
        ...
        for k, i in enumerate(IList):
            ...
            s = (
                ...
                * np.sqrt(
                    4.0 * np.cos(2 * I)
                    + 4 * np.cos(2 * nu + 2.0 * w)
                    - 2.0 * np.cos(-2 * I + 2.0 * nu + 2 * w)
                    - 2 * np.cos(2 * I + 2 * nu + 2 * w)
                    + 12.0
                )
                / 4.0
            )
            ...
            for col_name in data.columns:
                if (
                    col_name != "pl_orbincl"
                    and col_name != "pl_orbinclerr1"
                    and col_name != "pl_orbinclerr2"
                ):
                ...
                for count2, (l, band, bw, ws, wstep) in enumerate(bandzip):
                    pphi = (
                        photinterps2[float(feinterp(fe))][
                            float(distinterp(a / lum_fix))
                        ][c](beta.to(u.deg).value[inds], ws).sum(1)
                        * wstep
                        / bw
                    )[np.argsort(inds)]
            ...
            if orbdata is None:
                ...
                orbdata = orbdata._append(out)

            if orbitfits is None:
                orbitfits = pd.DataFrame(orbitfits_dict).copy()
            else:
                orbitfits = orbitfits._append(pd.DataFrame(orbitfits_dict))

    orbdata = orbdata.sort_values(by=["pl_id", "orbitfit_id", "t", "M"]).reset_index(
        drop=True
    )
...
def addEphemeris(data, orbfits, orbdata, bandzip, photdict):
    ...
    raw_text = np.genfromtxt(
        "plandb.sioslab.com/cache/planets_updated_param.txt", dtype=str, delimiter="\n"
    )
    planets = []
    for line_num, line_text in enumerate(raw_text):
        if "******" in line_text:
            planet_text = raw_text[line_num : line_num + 9]  # noqa
    ...
    data.loc[has_lum, "st_lum_correction"] = (
        10 ** data.loc[has_lum, "st_lum"]
    ) ** 0.5  # Since lum is log base 10 of solar luminosity
    ...
    # propagate filled in sma errors
    GMerrs = (
        ((data["st_masserr1"][nosma] - data["st_masserr2"][nosma]) / 2.0).values
        * u.solMass
        * const.G
    )
    Terrs = (
        (data["pl_orbpererr1"][nosma] - data["pl_orbpererr2"][nosma]) / 2.0
    ).values * u.day
    ...
    # update all WAs (and errors) based on sma
    WA = np.arctan(
        (data["pl_orbsmax"].values * u.AU) / (data["sy_dist"].values * u.pc)
    ).to("mas")
    data["pl_angsep"] = WA.value
    sigma_a = ((data["pl_orbsmaxerr1"] - data["pl_orbsmaxerr2"]) / 2.0).values * u.AU
    sigma_d = ((data["sy_disterr1"] - data["sy_disterr2"]) / 2.0).values * u.pc
    sigma_wa = (
        np.sqrt(
            (
                (data["pl_orbsmax"].values * u.AU) ** 2.0 * sigma_d**2
                + (data["sy_dist"].values * u.pc) ** 2.0 * sigma_a**2
            )
            / (
                (data["pl_orbsmax"].values * u.AU) ** 2.0
                + (data["sy_dist"].values * u.pc) ** 2.0
            )
            ** 2.0
        ).decompose()
        * u.rad
    ...
    merr = (
        (
            ((data["pl_bmassjerr1"].values - data["pl_bmassjerr2"].values) / 2.0)
            * u.M_jupiter
        )
        .to(u.M_earth)
        .value
    ...
    for j, m_val in enumerate(m):
        if np.isnan(merr[j]):
            ...
            merr_earth = (
                (
                    (
                        (data.iloc[j]["pl_bmasseerr1"] - data.iloc[j]["pl_bmasseerr2"])
                        / 2.0
                    )
                    * u.M_earth
                )
    ...
    tmpsmaserr = (
        data["pl_orbsmaxerr1"][noR].values - data["pl_orbsmaxerr2"][noR].values
    ) / 2.0
    adist = np.zeros((len(m), int(1e4)))
    for j, _ in enumerate(tmpsmas):  # Create smax distribution
        if np.isnan(tmpsmaserr[j]) or (tmpsmaserr[j] == 0):
    ...
    maxWA[hase] = np.arctan(
        (
            data["pl_orbsmax"][hase].values
            * (1 + data["pl_orbeccen"][hase].values)
            * u.AU
        )
        / (data["sy_dist"][hase].values * u.pc)
    ).to("mas")
    ...
    nolum_noteff_spect = (
        np.isnan(data["st_lum"].values) & ~data["st_spectype"].isnull().values
    )
    ...
    replacement_dict = {
        new_pl_id: old_pl_id
        for (old_pl_name, old_pl_id) in old_orbfits_id_dict.items()
        for (new_pl_name, new_pl_id) in new_orbfits_id_dict.items()
        if new_pl_name == old_pl_name
    }
...
def generateTables(data, orbitfits):
    ...
    colmap_st = {
        k: (
            ...
            if (
                k.startswith("st_")
                and (k != "st_id" and k != "st_name" and k != "st_radv")
            )
            else k
        )
...
def calcContrastCurves(data, exosims_json):
    ...
    for _, row in data.iterrows():
        ...
        star_base_path = Path(
            f'{contrast_curve_cache_base}/{star_name.replace(" ","_")}.p'
        )
    ...
    for mode in modes:
        ...
        inds = np.where(
            (data["pl_maxangsep"].values > minangsep.to(u.mas).value)
        ...
        working_angles_as = (
            np.linspace(minangsep.to(u.arcsec).value, maxangsep.to(u.arcsec).value)
            * u.arcsec
        )
        working_angles_lam_D = (working_angles_as.to(u.mas) / lam_D).value
        # working_angles_as = np.array(working_angles_as)
        ...
        for int_time in int_times:
            scenario_name = (
                f"{mode_name.replace(' ', '_')}_{int_time.to(u.hr).value:0.0f}hr"
            )
            scenario_path_list = []
            # star_F0s = []
            ...
            fEZ = TL.ZodiacalLight.fEZ(
                5.05, [20] * u.deg, 2.9 * u.AU
            )  # Taken from the Nemati spreadsheet
            ...
            for i, star_name in enumerate(tqdm(dup_star_names)):
                ind = dup_sInds[i]
                star_scenario_path = Path(
                    f'{contrast_curve_cache_base}/{star_name.replace(" ","_")}_{scenario_name}.p'
                )
                ...
                # contr_df['dMag'] = dMags
                # contr_df['r_lamD'] = None
                # contr_df['r_as'] = None
                # contr_df['r_mas'] = None
                contr_df["contrast"] = None
                contr_df["dMag"] = None
                contr_df["lam"] = [mode["lam"].value] * len(working_angles_lam_D)
                contr_df["t_int_hr"] = [int_time.value] * len(working_angles_as)
                contr_df["fpp"] = [
                    1 / TL.PostProcessing.ppFact(working_angles_as[0])
                ] * len(working_angles_as)
                contr_df.to_pickle(star_scenario_path)
            data[scenario_name] = scenario_path_list
    return data


def calcPlanetCompleteness(
    data, bandzip, photdict, exosims_json, minangsep=150, maxangsep=450, plotting=False
):
    ...
    scenario_angles = pd.read_csv("plandb.sioslab.com/cache/scenario_angles.csv")
    comp_scenarios = []
    compMinWA_scenarios = []
    compMaxWA_scenarios = []
    ...
    for ind, planet in tqdm(data.iterrows(), total=data.shape[0]):
        ...
        WAc, dMagc = np.meshgrid(
            WAbins0[:-1] + np.diff(WAbins0) / 2.0,
            dMagbins0[:-1] + np.diff(dMagbins0) / 2.0,
        )
        WAc = WAc.T
        dMagc = dMagc.T
        ...
        for scenario in scenario_names:
            ...
            if scenario_cc.exists():
                ...
                scenario_wfirstc = interp1d(
                    contr_df.r_mas, contr_df.contrast, bounds_error=False
                )
            else:
        ...
        astd = (planet["pl_orbsmaxerr1"] - planet["pl_orbsmaxerr2"]) / 2.0
        if np.isnan(astd):
            astd = 0.01 * amu
        ...
        if np.isnan(emu):
            gene = (
                lambda n: 0.175
                / np.sqrt(np.pi / 2.0)
                * np.sqrt(-2.0 * np.log(1 - np.random.uniform(size=n)))
            )
        else:
            estd = (planet["pl_orbeccenerr1"] - planet["pl_orbeccenerr2"]) / 2.0
        ...
        if np.isnan(Imu) or (
            (planet["pl_orbincl"] == 90)
            and (planet["pl_orbinclerr1"] == 0)
            and (planet["pl_orbinclerr2"] == 0)
        ):  # Generates full sinusoidal distribution if 90 incl 0 errors
            if planet["pl_bmassprov"] == "Msini":
                Icrit = np.arcsin(
                    ((planet["pl_bmassj"] * u.M_jupiter).to(u.M_earth)).value
                    / ((0.0800 * u.M_sun).to(u.M_earth)).value
                )
                Irange = [Icrit, np.pi - Icrit]
                C = 0.5 * (np.cos(Irange[0]) - np.cos(Irange[1]))
                genI = lambda n: np.arccos(
                    np.cos(Irange[0]) - 2.0 * C * np.random.uniform(size=n)
                )

            else:
                genI = lambda n: np.arccos(1 - 2.0 * np.random.uniform(size=n))
        else:
            Istd = (
                (planet["pl_orbinclerr1"] - planet["pl_orbinclerr2"])
                / 2.0
                * np.pi
                / 180.0
            )
        ...
        if np.isnan(wmu):
            ...
            wstd = (
                (planet["pl_orblpererr1"] - planet["pl_orblpererr2"])
                / 2.0
                * np.pi
                / 180.0
            )
        ...
        while np.any(pdiff > 0.0001) | (k < 3):
            ...
            if not np.isnan(planet.pl_radj):
                ...
                Rmu = planet.pl_radj
                Rstd = (planet["pl_radjerr1"] - planet["pl_radjerr2"]) / 2.0
                if np.isnan(Rstd):
                    Rstd = Rmu * 0.1
                ...
                if planet.pl_bmassprov == "Msini":
                    ...
                    Mstd = (
                        (
                            (planet["pl_bmassjerr1"] - planet["pl_bmassjerr2"])
                            * u.M_jupiter
                        ).to(u.M_earth)
                    ).value
                    if np.isnan(Mstd):
                        Mstd = (
                            (planet["pl_bmassj"] * u.M_jupiter).to(u.M_earth)
                        ).value * 0.1
                    Mp = (
                        np.random.randn(n) * Mstd
                        + ((planet["pl_bmassj"] * u.M_jupiter).to(u.M_earth)).value
                    )

                # Now use the Mp value to get the planet radius
                R = (
                    forecaster_mod.calc_radius_from_mass(Mp * u.M_earth)
                    .to(u.R_jupiter)
                    .value
                )
            ...
            for clevel in np.unique(cl):
                ...
                pphi[tmpinds] = (
                    photinterps2[float(feinterp(fe))][
                        float(distinterp(np.mean(rnorm) / lum_fix))
                    ][clevel](betatmp.to(u.deg).value[binds], ws).sum(1)
                    * wstep
                    / bw
            ...
            WA = (
                np.arctan((s * u.AU) / (planet["sy_dist"] * u.pc)).to("mas").value
            )  # working angle
                ...
                if wfirstc == None:
                    ...
                    currc = (
                        float(
                            len(
                                np.where(
                                    (WA >= minangseps[scenario_n])
                                    & (WA <= maxangseps[scenario_n])
                                    & (dMag <= dMaglimtmp)
                                )[0]
                            )
                        )
            ...
            if k == 1 and plotting:
                ...
                fig_path = Path(
                    "figures",
                    f'{mode_name.replace(" ", "_")}_{str(int_time).replace(" ", "_")}',
                )
                fig_path.mkdir(parents=True, exist_ok=True)
                save_path = Path(fig_path, f'{planet["pl_name"].replace(" ", "_")}.png')
                fig, ax = plt.subplots()
                ax.set_title(
                    f'Mode: {mode_name.replace("EB", "Conservative").replace("DRM", "Optimistic").replace("_", " ")}. Int time: {int_time}. Planet: {planet["pl_name"]}'
                )
                ...
                H, WA_edges, dMag_edges = np.histogram2d(
                    WA,
                    dMag,
                    range=[
                        [WA_edges[0], WA_edges[-1]],
                        [dMag_edges[0], dMag_edges[-1]],
                    ],
                    bins=(500, 500),
                )
...
def genAliases(data):
    ...
    for j, star in enumerate(starnames):
        ...
        if (
            (len(noipacalias) > 0)
            and (len(nosimbadalias) > 0)
            and (noipacalias[-1] == star)
            and (nosimbadalias[-1] == star)
        ):
...
def fillMissingAliases(connection, data):
    result = connection.execute("select Alias from Aliases where NEAName=1")
...
def writeSQL_old(
    connection, data=None, orbdata=None, altorbdata=None, comps=None, aliases=None
):
    ...
    if data is not None:
        ...
        # set indexes
        result = connection.execute("ALTER TABLE KnownPlanets ADD INDEX (pl_name)")
        result = connection.execute("ALTER TABLE KnownPlanets ADD INDEX (pl_hostname)")

        # add comments
        addSQLcomments(connection, "KnownPlanets")
    ...
    if orbdata is not None:
        ...
        result = connection.execute("ALTER TABLE PlanetOrbits ADD INDEX (Name)")
        ...
        addSQLcomments(connection, "PlanetOrbits")
    ...
    if aliases is not None:
        ...
        result = connection.execute("ALTER TABLE Aliases ADD INDEX (Alias)")
        result = connection.execute("ALTER TABLE Aliases ADD INDEX (SID)")
...
def writeSQL(
    ...
    connection.execute(
        text(
            "DROP TABLE IF EXISTS Completeness, ContrastCurves, Scenarios, PDFs, Orbits, OrbitFits, Planets, Stars"
        )
    )
    ...
    if plandata is not None:
        ...
        result = connection.execute(
            text(
                "ALTER TABLE Planets ADD FOREIGN KEY (st_id) REFERENCES Stars(st_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
    ...
    if orbitfits is not None:
        ...
        result = connection.execute(
            text("ALTER TABLE OrbitFits ADD INDEX (orbitfit_id)")
        )
        result = connection.execute(text("ALTER TABLE OrbitFits ADD INDEX (pl_id)"))
        result = connection.execute(
            text(
                "ALTER TABLE OrbitFits ADD FOREIGN KEY (pl_id) REFERENCES Planets(pl_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
    ...
    if orbdata is not None:
        ...
        result = connection.execute(
            text(
                "ALTER TABLE Orbits ADD FOREIGN KEY (pl_id) REFERENCES Planets(pl_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
        result = connection.execute(
            text(
                "ALTER TABLE Orbits ADD FOREIGN KEY (orbitfit_id) REFERENCES OrbitFits(orbitfit_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
    ...
    if pdfs is not None:
        ...
        result = connection.execute(
            text(
                "ALTER TABLE PDFs ADD FOREIGN KEY (pl_id) REFERENCES Planets(pl_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
    ...
    if aliases is not None:
        ...
        result = connection.execute(
            text(
                "ALTER TABLE Aliases ADD FOREIGN KEY (st_id) REFERENCES Stars(st_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
    ...
    if scenarios is not None:
        ...
        scenarios.to_sql(
            ...
            if_exists="replace",
            dtype={
                "scenario_name": sqlalchemy.types.String(namemxchar),
            },
            index=False,
        )

        result = connection.execute(
            text("ALTER TABLE Scenarios ADD INDEX (scenario_name)")
        )
    ...
    if contrastCurves is not None:
        ...
        namemxchar = np.array(
            [len(n) for n in contrastCurves["scenario_name"].values]
        ).max()
        contrastCurves.to_sql(
            "ContrastCurves",
            connection,
            chunksize=100,
            if_exists="replace",
        ...
        result = connection.execute(
            text(
                "ALTER TABLE ContrastCurves ADD FOREIGN KEY (st_id) REFERENCES Stars(st_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
        result = connection.execute(
            text(
                "ALTER TABLE ContrastCurves ADD FOREIGN KEY (scenario_name) REFERENCES Scenarios(scenario_name) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
    ...
    if completeness is not None:
        ...
        namemxchar = np.array(
            [len(n) for n in completeness["scenario_name"].values]
        ).max()
        completeness.to_sql(
            "Completeness",
            connection,
        ...
        result = connection.execute(
            text(
                "ALTER TABLE Completeness ADD FOREIGN KEY (pl_id) REFERENCES Planets(pl_id) ON DELETE NO ACTION ON UPDATE NO ACTION"
            )
        )
        result = connection.execute(
            text(
...
def addSQLcomments(connection, tablename):
    ...
    result = connection.execute("show create table %s" % tablename)
    res = result.fetchall()
    ...
    for key, d in zip(keys, defs):
        ...
        comm = """ALTER TABLE `%s` CHANGE `%s` %s COMMENT "%s %s";""" % (
            ...
            key,
            d,
            cnames[cols == key][0].strip('"'),
            cdefs[cols == key][0],
        )
        print(comm)
        r = connection.execute(comm)
