...
def pointingAngles(tgt_in, SC_loc=None):
    ...
    # for plotting vectors in inertial coordinates
    xpp = np.matmul(
        np.transpose(mat_C2), np.matmul(np.transpose(mat_C1), np.array([1, 0, 0]))
    )
    ypp = np.matmul(
        np.transpose(mat_C2), np.matmul(np.transpose(mat_C1), np.array([0, 1, 0]))
    )
    zpp = np.matmul(
        np.transpose(mat_C2), np.matmul(np.transpose(mat_C1), np.array([0, 0, 1]))
    )
    ...
    xppp = np.matmul(
        np.transpose(mat_C2),
        np.matmul(
            np.transpose(mat_C1), np.matmul(np.transpose(mat_C3), np.array([1, 0, 0]))
        ),
    )
    yppp = np.matmul(
        np.transpose(mat_C2),
        np.matmul(
            np.transpose(mat_C1), np.matmul(np.transpose(mat_C3), np.array([0, 1, 0]))
        ),
    )
    zppp = np.matmul(
        np.transpose(mat_C2),
        np.matmul(
            np.transpose(mat_C1), np.matmul(np.transpose(mat_C3), np.array([0, 0, 1]))
        ),
    )
    ...
    # plotting various unit vectors
    ax.plot(r_so[0], r_so[1], r_so[2], marker="*", mfc="r", mec="r")  # sun is red star
    ax.plot(
        r_eo[0], r_eo[1], r_eo[2], marker=".", mfc="b", mec="b"
    )  # earth is blue dot
    ax.plot(
        r_sc_o[0], r_sc_o[1], r_sc_o[2], marker="D", mfc="none", mec="m"
    )  # sc is magneta diamond
    ax.plot(
        tgt_ec[0], tgt_ec[1], tgt_ec[2], marker="X", mfc="g", mec="g"
    )  # tgt is green X
    ...
    # plotting old vs new unit vectors in 1 transformation - below is for 4th rotation (pitch4)
    # old
    ptx = xppp * tnorm
    pty = yppp * tnorm
    ptz = zppp * tnorm
    ax.plot(
        np.array([r_sc_o[0], r_sc_o[0] + ptx[0]]),
        np.array([r_sc_o[1], r_sc_o[1] + ptx[1]]),
        np.array([r_sc_o[2], r_sc_o[2] + ptx[2]]),
        "r-",
        label="x_old",
    )
    ax.plot(
        np.array([r_sc_o[0], r_sc_o[0] + pty[0]]),
        np.array([r_sc_o[1], r_sc_o[1] + pty[1]]),
        np.array([r_sc_o[2], r_sc_o[2] + pty[2]]),
        "g-",
        label="y_old",
    )
    ax.plot(
        np.array([r_sc_o[0], r_sc_o[0] + ptz[0]]),
        np.array([r_sc_o[1], r_sc_o[1] + ptz[1]]),
        np.array([r_sc_o[2], r_sc_o[2] + ptz[2]]),
        "b-",
        label="z_old",
    )
